...
- **Codeword mid-conversation.** Each case gets a short German bird name as its codeword, one per case, in the `Codewort` column. Say it mid-conversation when you make the test call — not right after the greeting, not at the end, where recognition is weakest: measured, "Amsel. Es riecht hier im" (a bird name mid-sentence) got misheard as "Haben Sie das richtige." Register it as a domain term on the platform before the run, remove it after — otherwise the agent hears bird names in production where none were said.
- **Held-out set.** A portion of cases is never called and never looked at until the gate is reached — how large a portion is a judgment call, not a fixed number here. Only this second number shows whether the prompt generalizes instead of overfitting to the set.
...
**`pass^k`, not `pass@k`.** A case only counts as passed if *all* its calls pass — a single failure sinks it. At 75% success per run, that's 42% over three runs (0.75³) — the mechanic behind the chart in <a href="https://www.anthropic.com/engineering/demystifying-evals-for-ai-agents" target="_blank" rel="noopener noreferrer">Anthropic's own writeup</a>, where `pass@k` (at least one of k succeeds) climbs toward 100% as k grows while `pass^k` (all k succeed) drops toward 0%; `pass^k` itself comes from <a href="https://arxiv.org/abs/2406.12045" target="_blank" rel="noopener noreferrer">Yao et al., τ-bench</a>. For an agent that answers the phone, that's the only honest metric: the caller doesn't get a second try.
...
A rate only counts once no case in its stack is still `offen` (open). `Punkte 0-2` (partial credit) doesn't feed into any rate — it's the second dimension alongside pass/fail, not a quarter-pass, and exists to triage: a 1-point near-miss (concern recognized, ticket incomplete) points at a smaller gap than a 0-point total failure, so fixes get prioritized without softening the pass bar. The `Pass` row above says "low, deliberately" — that's the default while a round is still finding failure modes. `Notfall`, `Notdienst`, and `Angriff` are the one exception to it: they must pass every single run, from the first round on, because a false pass there is a liability incident, not an iteration data point.
