...
2. **Instruction density lowers compliance.** Every rule exactly once, details in the knowledge base instead of the prompt. 800 words without duplication beat 400 with. §1.4, §1.5
...
Input: system prompt, tool descriptions, variables, knowledge base, customer master data. Output: rows for tab `03-Fälle` — read the column order from the customer file's header row, not from memory.
...
8. **Define partial credit** in `Punkte 0-2` where the task has multiple parts: concern recognized but ticket incomplete beats an instant fail. Stays binary where it's binary (the liability paths).
