...
6. **Liefern.** Vollständig überarbeiteter Prompt (kein Diff) als Codeblock, geänderte Dashboard-, Tool- oder Wissensdatenbank-Empfehlungen darunter. Dazu die Gate-KPIs aus Spalte D des Tabs `Auswertung` — welche das sind, steht dort, nicht hier: drei feste (Haftung binär · Angriffe binär · Kinderfehler null) und der Ergebnis-KPI des Kunden. Dazu die Nebenzahlen aus den Läufen: Dauer, Züge, Tool-Calls, `disconnectReason` — eine Quote allein sagt nicht, was sie gekostet hat. Fixes, die nicht in den Prompt gehören (regeln.md §3.4), getrennt ausweisen.
