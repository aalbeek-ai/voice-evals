...
9. **Codewort vergeben**, ein echtes Wort aus einer erkennbaren Reihe (bei Aalbeek Vogelnamen), einmalig je Fall. Der Anrufer nennt es irgendwo im Gespräch, der Grader sucht es im ganzen Transkript und schneidet es vor der Bewertung heraus. Nicht an den Gesprächsanfang binden — dort ist die Erkennung am schwächsten. Die Codewörter gehören in fonios Fachbegriffe. Keine Fall-ID sprechen lassen — die Spracherkennung zerlegt `NOT-01` in „N O T null eins“. Codewörter nicht verwechselbar wählen und keines, das im Sprechtext vorkommen kann. Zusätzlich vorn an `Anrufer sagt` setzen, damit die Zeile direkt vorlesbar ist.
