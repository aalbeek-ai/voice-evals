...
9. **Codewort vergeben**, ein echtes Wort aus einer erkennbaren Reihe (bei Aalbeek Vogelnamen), einmalig je Fall. Der Anrufer nennt es irgendwo im Gespräch, der Grader sucht es im ganzen Transkript und schneidet es vor der Bewertung heraus. Nicht an den Gesprächsanfang binden — dort ist die Erkennung am schwächsten. Kein Eintrag im fonio-Dashboard nötig, solange die Wörter kurz und geläufig sind (Amsel, Möwe, Specht) — keine seltenen oder ähnlich klingenden. Keine Fall-ID sprechen lassen — die Spracherkennung zerlegt `NOT-01` in „N O T null eins“. Codewörter nicht verwechselbar wählen und keines, das im Sprechtext vorkommen kann. Nicht in `Anrufer sagt` schreiben — das Codewort ist Testmechanik, kein Teil des Anliegens.
